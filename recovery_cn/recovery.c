...
static const char *BATTERY_CAPACITY = "/sys/class/power_supply/battery/capacity";
...
static void draw_battery_top_right() {
    char * file_data;
    char tmp[PATH_MAX];
	char batteryDisplay[PATH_MAX];
    FILE * f;
    int line=0;

	// focus.lau comment
	// charing or not   : /sys/class/power_supply/battery/status
	// battery capacity : /sys/class/power_supply/battery/capacity
	// battery health   : /sys/class/power_supply/battery/health

    f = fopen_path(BATTERY_CAPACITY, "rb");
    if (f != NULL) {
        file_data = fgets(tmp, PATH_MAX, f);
        if (file_data != NULL) {
			ui_print("battery:%s\n", tmp);
		}
		ui_print("battery:none\n");
        check_and_fclose(f, BATTERY_CAPACITY);
    }
}
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case ITEM_WIPE_CACHE:
                if (confirm_selection("确定要擦除吗?", "是 - 擦除缓存"))
                {
                    ui_print("\n-- 正在擦除缓存...\n");
                    erase_volume("/cache");
                    ui_print("缓存擦除完成.\n");
